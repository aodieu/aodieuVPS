#!/bin/bash
#######################################################
# aodieuVPS Script v1.0 for Rocky Linux 8
# To install type: 
# curl -sO https://aodieu.com/script/install && bash install
# or
# curl -sO https://aodieu.com/script/$(rpm -E %rhel)/install && bash install
#######################################################
...
script_url="https://aodieu.com/script/scripts/8"
...
if [ $server_ram_total -lt $low_ram ]; then
	echo -e "Canh bao: dung luong RAM qua thap de cai aodieuVPS Script \n (it nhat 256MB) \n"
	echo "huy cai dat..."
	exit
fi
sleep 3
...
select opt in "${options[@]}"; do 
    ...
    case "$REPLY" in
    1) php_version="8.4"; break;;
    2) php_version="8.3"; break;;
    3) php_version="8.2"; break;;
    4) php_version="8.1"; break;;
    5) php_version="8.0"; break;;
...
read server_name
if [ "$server_name" = "" ]; then
	server_name="aodieu.com"
	echo "Ban nhap sai, he thong dung aodieu.com lam ten mien chinh"
fi
...
dnf -y remove mysql* php* httpd* sendmail* postfix* rsyslog*
dnf clean all
dnf -y update
...
if [ "$php_version" = "8.4" ]; then
	...
	dnf -y install nginx php-fpm php-common php-gd php-mysqlnd php-pdo php-xml php-mbstring php-curl php-opcache php-cli php-pecl-zip php-intl php-soap
	...
	dnf -y install nginx php-fpm php-common php-gd php-mysqlnd php-pdo php-xml php-mbstring php-mcrypt php-curl php-opcache php-cli php-pecl-zip php-intl php-soap
	...
	dnf -y module enable php:remi-7.1
	dnf -y install nginx php-fpm php-common php-gd php-mysqlnd php-pdo php-xml php-mbstring php-mcrypt php-curl php-opcache php-cli php-pecl-zip php-intl php-soap
elif [ "$php_version" = "7.0" ]; then
	dnf -y module enable php:remi-7.0
	dnf -y install nginx php-fpm php-common php-gd php-mysqlnd php-pdo php-xml php-mbstring php-mcrypt php-curl php-opcache php-cli php-pecl-zip php-intl php-soap
else
	dnf -y install nginx php-fpm php-common php-gd php-mysqlnd php-pdo php-xml php-mbstring php-curl php-opcache php-devel php-cli gcc
fi
...
server {
	...
	auth_basic_user_file /home/$server_name/private_html/aodieuvps/.htpasswd;
...
mysql mysql < mariadb10_3tables.sql 2>/dev/null

systemctl restart mariadb.service
mariadb-upgrade --force 2>/dev/null || mysql_upgrade --force mysql
...
# Install snapd for Rocky Linux 8
dnf install snapd -y
...
# Run certbot
certbot --nginx --non-interactive --agree-tos --register-unsafely-without-email || echo "Certbot failed or skipped"
...
mv -f administrator/* .
rm -rf administrator
printf "admin:$(openssl passwd -apr1 $admin_password)\n" > /home/$server_name/private_html/aodieuvps/.htpasswd
sed -i "s/rootpassword/$root_password/g" /home/$server_name/private_html/aodieuvps/SQLManager.php
...
mkdir -p /etc/aodieuvps/

cat > "/etc/aodieuvps/scripts.conf" <<END
aodieuvps_version="$aodieuvps_version"
server_name="$server_name"
server_ip="$server_ip"
...
END
chmod 600 /etc/aodieuvps/scripts.conf

clear
printf "=========================================================================\n"
printf "Cau hinh hoan tat, bat dau them menu aodieuvps, nhanh thoi... \n"
printf "=========================================================================\n"

wget -q $script_url/aodieuvps -O /bin/aodieuvps && chmod +x /bin/aodieuvps
mkdir /etc/aodieuvps/menu/
cd /etc/aodieuvps/menu/
wget -q $script_url/component/menu.zip
unzip -q menu.zip && rm -f menu.zip
chmod +x /etc/aodieuvps/menu/*

clear
cat > "/root/aodieuvps-script.txt" <<END
=========================================================================
                           MANAGE VPS INFORMATION                        
=========================================================================
Lenh truy cap menu aodieuVPS Script: aodieuvps

Domain chinh: http://$server_name/ hoac http://$server_ip/

aodieuVPS Script Admin:	http://$server_name:$admin_port/ hoac http://$server_ip:$admin_port/
...
Neu can ho tro, cac ban hay truy cap https://aodieu.com/script/
END

chmod 600 /root/aodieuvps-script.txt

if [ "$1" = "wordpress" ]; then
	printf "=========================================================================\n"
	printf "Hoan tat qua trinh cai dat aodieuVPS Script + WordPress! \n"
	printf "=========================================================================\n"
	printf "Tiep theo ban hay truy cap http://$server_name \n hoac http://$server_ip de cau hinh WordPress \n"
else
	printf "=========================================================================\n"
	printf "Scripts aodieuVPS da hoan tat qua trinh cai dat... \n"
	printf "=========================================================================\n"
...
printf "=========================================================================\n"
printf "aodieuVPS Script Admin: http://$server_name:$admin_port/ \n hoac http://$server_ip:$admin_port/\n\n"
...
printf "=========================================================================\n"
printf "Thong tin quan ly duoc luu tai: /root/aodieuvps-script.txt \n"
printf "=========================================================================\n"
printf "***Luu y: Port dang nhap SSH da duoc doi tu 22 sang 2222 de bao mat VPS\n"
printf "=========================================================================\n"
printf "De quan ly server, ban hay dung lenh \"aodieuvps\" khi ket noi SSH.\n"
printf "Neu can ho tro, cac ban hay truy cap https://aodieu.com/script/\n"
printf "=========================================================================\n"
